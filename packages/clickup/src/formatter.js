(function (factory) {
    ...
    exports.formatTeamSync = formatTeamSync;
    /**
     ...
     * @returns Markdown-formatted string.
     */
    function formatTeamSync(params) {
        ...
        for (var so = 0; so < stageOrder.length; so++) {
            ...
            if (stageName === "Blocked") {
                ...
                for (var bt = 0; bt < sorted.length; bt++) {
                    var bTask = sorted[bt];
                    lines.push("| " + escapeCell(bTask.name) +
                ...
                for (var st = 0; st < sorted.length; st++) {
                    var sTask = sorted[st];
                    lines.push("| " + escapeCell(sTask.name) +
        ...
        if (unassigned.length > 0) {
            ...
            for (var u = 0; u < unassigned.length; u++) {
                ...
                var priorityLabel = uTask.priority && uTask.priority.priority
                    ...
                    : "-";
                lines.push("| " + escapeCell(uTask.name) +
                    " | " + priorityLabel +
        ...
        if (unmappedKeys.length > 0) {
            ...
            for (var uk = 0; uk < unmappedKeys.length; uk++) {
                ...
                lines.push("- \"" + uStatus + "\" (" + unmappedStatuses[uStatus] +
    ...
    function daysStalled(updatedTs, now) {
        if (!updatedTs) {
            return "?";
        }
