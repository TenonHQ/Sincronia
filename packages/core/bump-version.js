...
function bumpVersion(packagePath) {
  try {
    ...
    if (versionParts.length !== 3) {
      console.error(`Invalid version format: ${currentVersion}. Expected format: major.minor.patch`);
      process.exit(1);
    }
...
function findPackageJson() {
  ...
  if (fs.existsSync(sincConfigPath)) {
    try {
      ...
      if (sincConfig.scopes) {
        const scopeName = process.env.SINC_SCOPE || Object.keys(sincConfig.scopes)[0];
        ...
        if (scope && scope.sourceDirectory) {
          ...
          if (fs.existsSync(scopePackagePath)) {
            console.log(`Using scope-based package.json for scope: ${scopeName}`);
  ...
  if (!fs.existsSync(defaultPath)) {
    console.error('No package.json found. Please specify the path as an argument.');
